...
- **Area (hectares):** PARIVESH's own `project_area` is unusable for two of the three
  types — **blank on 100% of EC records and the literal string `0` on 100% of CRZ**;
  only FC carries a real figure (99.9% populated). Area is therefore **derived from
  geometry** — polygon area on the WGS84 ellipsoid via `pyproj.Geod`, converted to
  hectares.

  **It is computed and published in the data, but deliberately not displayed on the map.**
  Checked against FC, the one layer where PARIVESH states an area, the derived figure has
  a median ratio of 1.009 — so the computation is right — but only 47% land within ±10%
  and 32% exceed the stated area by more than 2×. The submitted boundary and the land
  actually cleared are not the same quantity, and how far they diverge appears to depend
  on how the individual proponent drew the KML rather than on project type. Compounding
  it: ~10% of EC and ~25% of CRZ polygons are under 0.01 ha (not real footprints), and some
  records carry district-scale boundaries — 111 EC proposals exceed 100,000 ha, the
  largest being 972,369 ha (a District Survey Report covering a whole district). A single number labelled "Area"
  would imply a precision the source does not support, so the map shows none. **Never sum
  it**: EC alone totals ~590,000 km², about 18% of India's land area.
...
| 3 | year granted | 2025 |
| 4 | area (hectares) — carried in the data, not shown on the map (§3.3) | 1.0 |
